...
let
  cfg = config.services.coredhcp;
  configFile = pkgs.writeText "config.yaml" cfg.config;
in {
  options.services.coredhcp = {
    enable = mkEnableOption "CoreDHCP DHCP server";
    ...
    config = mkOption {
      ...
      type = types.lines;
      description = ''
        Example coreDHCP configuration.'';
    };

    package = mkOption {
      default = pkgs.coredhcp;
      defaultText = "pkgs.coredhcp";
      type = types.package;
      description = "coredhcp package to use.";
    };
  };

  config = mkIf cfg.enable {
    systemd.services.coredhcp = {
      ...
      serviceConfig = {
        ...
        DynamicUser = true;
        ExecStart = "${getBin cfg.package}/bin/coredhcp -conf=${configFile}";
        ExecReload = "/bin/kill -SIGUSR1 $MAINPID";
        StateDirectory = "coredhcp";
        Restart = "on-failure";
      };
    };
  };
